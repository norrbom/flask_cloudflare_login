...
dash.register_page(
    __name__, path='/profile', title=f"{getenv('APP_NAME')} - Profile"
    )


def layout():
    profile = [
        key + ': ' + str(current_user.__dict__()[key]) for key in current_user.__dict__()
        ]
            ...
            html.Div(
                html.Div(
                    children=[
                        html.Ul(
                            id='profile-list',
                            children=[html.Li(p) for p in profile]
                        )
                    ],
                )
            )
